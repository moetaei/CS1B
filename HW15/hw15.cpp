...
int main()
{
    ...
    if (p==myList.end())    // did we reach the end?
        cout << "low alg: the list is empty" << endl;
    else
...
// Q#1 An STL container is a set of classes whose purpose is to contain other objects. They are all templated classes and can be instantiated to contain any type of object. An STL iterator is a generalization of pointers to be able to traverse a container with any type. An STL algorithm is the way you can manipulate the data stored within the containers. An exmaple of an STL container would be vector. An STL iterator would be vector.begin() and vector.end(). An example of an STL algorithm would be the reverse algorithm.

// Q#2 The iterator architecture makes it possible to decouple algorithms from containers by operating on a range of elemnets rather than on the container itself. This is important because its the most generalized way you can have an iterator and be able to traverse and list or container, regardless of type, making the iterator very useful.

// Q#5 low_doubles has issues bbecause of the l and low variables. The algorithm holds two variables whose values are constantly being changed to compare the lowest value. The low algorithm holds one low value and uses an iterator, making it point to data via iterator and using the iterator as the means of access for the data, making it slightly more efficient as an algorithm. The values differ in the algorithms because the low_doubles algorithm uses -1 as a value thats low and that value could not even be part of the list, and if no number is less than -1, the algorithm will return a -1, which would be incorrect.
